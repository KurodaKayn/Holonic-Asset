...
import type { AudioStudioController } from "./state/use-audio-studio";

export function AudioStudioWorkspace({
  studio,
}: {
  studio: AudioStudioController;
}) {
  const { duration, masterMuted, playing, prompt, time, tracks } = studio.model;
  const { actionError, isLoading, isMutating, loadError } = studio.status;
  const {
    addTrack,
    generateVariation,
    reload,
